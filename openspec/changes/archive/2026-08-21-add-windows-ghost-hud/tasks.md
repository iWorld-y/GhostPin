...
- [x] 6.2 人工验证默认穿透点击下层应用、穿透态不抢焦点、交互态按钮/拖动/八方向缩放、置顶开关、任务栏与 Alt+Tab 隐藏以及托盘生命周期
- [x] 6.3 在 100%、150%、200% DPI 和含负坐标的双显示器组合验证移动、重启恢复、显示器断开回退、文字清晰度和命中区域
- [x] 6.4 人工验证外部创建、修改、原子替换、短暂损坏和恢复 `todos.json` 时 HUD 在数秒内稳定更新、不闪烁且不改变其他应用焦点
