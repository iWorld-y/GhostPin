...
- 不引入 Windows CLI、提醒通知、安装器、自动更新、开机启动或发布流水线。
- 不追求跨框架逐像素复刻；Windows HUD 与设置页需要沿用 macOS 的浅色材质、白绿黄渐变、系统表单层级与模式提示。
...
### 4. WPF 视觉沿用 macOS 色调，不依赖系统材质

HUD 根窗口保持完全透明，内容板使用与 macOS 相同方向的白色材质、绿色与黄色半透明渐变、圆角 Border、克制阴影和品牌 PNG；正文改用深色主文字与灰色次要文字，避免形成固定黑色主题。卡片沿用 Doing/Todo 分区、标题、描述、优先级和截止时间层级。交互模式通过描边、亮度、卡片按钮可见性和短动画区分；穿透模式降低饱和度与透明度。设置窗口使用浅色系统背景、白色分组卡片和标准控件，结构与 macOS 的 HUD/高级页签一致。
...
### 9. 通知区域承载高频入口，独立设置窗口承载详细偏好

应用启动后创建一个 HUD 窗口和一个通知区域图标。通知区域菜单与 macOS 状态栏菜单保持相同的基本层级，通过 `HudController` 调用显示/隐藏、模式切换、打开设置和退出；菜单文本与勾选状态由同一应用状态生成。透明度、范围、条数上限和置顶放入单实例 WPF 设置窗口，控件修改后立即调用现有 Controller 设置方法，不复制持久化逻辑。

通知区域继续使用 .NET 自带的 WinForms `NotifyIcon` 作为 Shell 薄封装，但图标从现有 `GhostPinStatusBar.png` WPF 资源转换为拥有独立句柄生命周期的 `System.Drawing.Icon`，不得回退到 `SystemIcons.Application`。这样既复用 macOS 状态栏的品牌轮廓，也不引入额外 `.ico` 源文件或第三方图像库。

关闭设置窗口只隐藏该窗口，进程继续运行；再次打开时同步当前设置并激活已有实例。隐藏 HUD 仍只调用 Hide 并保存可见性，进程继续监听任务文件。退出路径按“停止 watcher → 保存设置 → 关闭设置窗口 → 移除通知区域图标 → 关闭 HUD 和 Dispatcher”的顺序执行。

### 10. 全局快捷键复用 macOS 的录制与提交语义

设置窗口高级页允许用户录制用于切换 HUD 穿透/交互模式的全局快捷键。普通按键必须包含 Ctrl、Alt、Shift 或 Win 修饰键，F1–F20 可单独使用，修饰键本身不构成候选；Esc 取消录制。

平台层通过公开 `RegisterHotKey`、`UnregisterHotKey` 和 `WM_HOTKEY` 实现注册与分发，并用 `MOD_NOREPEAT` 避免长按重复切换。开关打开但尚无候选时只进入待配置状态；只有候选成功注册后才保存快捷键和启用状态。替换已启用快捷键前先暂时注销旧组合；新组合注册失败时恢复旧组合并显示原因，确保设置文件不会持久化不可用候选。

### 11. 自动化验证聚焦领域与可分离平台策略
