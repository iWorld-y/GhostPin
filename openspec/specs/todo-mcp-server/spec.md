...
`list_tasks` SHALL 查询任务；`include_completed` 参数为 false 或省略时只返回未完成任务，为 true 时返回全部任务；返回的任务对象 MUST 包含 id、title、createdAt、completedAt、source、reminderAt、reminderSentAt、isCompleted 八个字段，日期为 ISO8601 字符串。

#### Scenario: 查询未完成任务

- **WHEN** 客户端调用 list_tasks 且未传 include_completed
- **THEN** 返回全部未完成任务，按创建时间从新到旧，每项八字段完整
...
`create_task` SHALL 创建任务，参数为 `title`（必填）与 `reminder_at`（可选，ISO8601 字符串）；不解析自然语言时间；非法 reminder_at MUST 返回工具错误且不创建任务。

#### Scenario: 创建纯文本任务

- **WHEN** 客户端调用 create_task 且仅提供 title
- **THEN** 创建任务，返回八字段任务对象，无提醒时间
...
`update_task` SHALL 修改任务：`title` 改标题、`reminder_at` 改提醒时间、`clear_reminder` 清除提醒，三者 MUST 至少提供一项；只提供部分参数时未提供字段保持不变。
...
- **WHEN** 客户端调用 update_task 且未提供 title、reminder_at 或 clear_reminder
- **THEN** 返回 isError 工具错误，任务数据不变
