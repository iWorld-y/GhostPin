...
定义 `ghostpin-cli` 命令行工具的命令集、显式参数与输出契约，使 Agent（OpenCode、Codex 等）能够通过 CLI 稳定地查询与增删改任务，不依赖 App 是否运行。
