...
- **全局快捷键可选（默认关闭）**：默认不注册全局快捷键；可在设置「高级」页为穿透/交互切换录制一个全局快捷键。保存前做系统快捷键预检，注册失败会阻止保存；第三方软件的快捷键冲突无法完全自动检测。
...
- 菜单栏托盘：未完成数量徽标、显示/隐藏桌面便签、交互模式开关、设置、退出。设置包含 HUD 页与「高级」页（可选全局快捷键）。
